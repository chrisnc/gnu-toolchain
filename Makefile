...
$(binutils_build)/Makefile: binutils
	...
	cd $(binutils_build) && \
	CXXFLAGS="-g -O2 -std=gnu++14" "$(topdir)/binutils/configure" --quiet --target="$(target)" --prefix="$(prefix)" --with-sysroot="$(sysroot)" --disable-nls --disable-werror --disable-sim --disable-gdb --enable-ld=default --enable-gold=yes --enable-multilib --enable-plugins --enable-lto
